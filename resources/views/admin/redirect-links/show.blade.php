...
@section('header')
    {{ $redirectLink->name }}
    <span class="text-base tinos-regular text-gray-500 ml-2">/r/{{ $redirectLink->code }}</span>
@endsection
...
<div class="mb-6 flex items-center justify-between">
    <div class="text-sm text-gray-600 tinos-regular">
        Redirects to: <a href="{{ $redirectLink->destination_url }}" target="_blank" class="text-black hover:underline">{{ Str::limit($redirectLink->destination_url, 60) }}</a>
    </div>
    <div class="flex gap-3">
        <a href="{{ route('redirect-links.edit', $redirectLink) }}" class="bg-white text-black border-black border border-r-3 border-b-3 px-4 py-2 text-sm tinos-bold">Edit</a>
        <a href="{{ route('redirect-links.index') }}" class="text-gray-600 hover:text-black text-sm tinos-regular py-2">Back to list</a>
    </div>
</div>
